...
function buildSummaryTable(){
  const totalDonations = donors.reduce((sum, donor)=> sum += donor[9], 0);
  const summaryTable = `<table><tr><th>Donors</th><td>${donors.length}</td></tr><tr><th>Total Donations</th><td>$ ${totalDonations}</td></th></table>`;
  document.getElementById("donationSummary").innerHTML = summaryTable;
}

function buildMajorDonorsTable(){
  const majorDonors = donors.filter(findMajorDonors);
  majorDonors.sort(donorSortDescending);
  const donorTable = `<table><caption>Major Donors</captions><thead><tr><th>Donation</th><th>Donor ID</th><th>Date</th><th>Name</th><th>Address</th><th>Phone</th><th>E-mail</th></tr></thead><tbody>${majorDonors.reduce(writeDonorRow, '')}</tbody></table>`;
...
function writeDonorRow(str, donor) {
   str += "<tr>";
   str += "<td>$" + donor[9].toLocaleString() + "</td>";   
   str += "<td>" + donor[0] + "</td>";
   str += "<td>" + donor[10] + "</td>";   
   str += "<td>" + donor[2] + ", " + donor[1] + "</td>";  
   str += "<td>" + donor[3] + "<br />" + donor[4] + ", " + donor[5] + " " + donor[6]  + "</td>";    
   str += "<td>" + donor[7] + "</td>";   
   str += "<td>" + donor[8] + "</td>";         
   str += "</tr>";
  return str;
